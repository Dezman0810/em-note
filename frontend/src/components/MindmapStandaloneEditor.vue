  ...
  win.postMessage(
    {
      ...
      data: parseMindmapScene(props.scene),
...
function onFrameMessage(ev: MessageEvent) {
  ...
  if (msg.type !== 'em-note-mindmap-save' || props.readOnly) return
  const json = stringifyMindmapScene(msg.data ?? {})
  if (normalizeSceneJson(json) === normalizeSceneJson(lastEmittedScene.value || props.scene)) return
