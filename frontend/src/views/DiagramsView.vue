...
import { errMessage, diagramsApi } from '../api/client'
import type { NoteDiagramDetail, NoteDiagramListItem } from '../api/types'
import NoteEditorColumn from '../components/NoteEditorColumn.vue'
import DrawioStandaloneEditor from '../components/DrawioStandaloneEditor.vue'
...
const LIST_COLLAPSED_KEY = 'note-ui-diagrams-list-collapsed'
const LIST_WIDTH_KEY = 'note-ui-diagrams-list-width'
const SCHEMA_COL_KEY = 'note-ui-diagrams-col-name'
const LIST_MIN = 220
const LIST_MAX = 760
...
const items = ref<NoteDiagramListItem[]>([])
...
const detail = ref<NoteDiagramDetail | null>(null)
const detailLoading = ref(false)
const saveError = ref('')
const lastMap = ref<{ noteId: string; index: number } | null>(null)
const sceneEditorRef = ref<InstanceType<typeof DrawioStandaloneEditor> | null>(null)

const showingSchema = computed(() => route.name === 'diagram-edit')
const showingNote = computed(() => route.name === 'diagram-note')
...
function rowKey(row: Pick<NoteDiagramListItem, 'note_id' | 'schema_index'>) {
  return `${row.note_id}:${row.schema_index}`
}

function isSchemaSelected(row: NoteDiagramListItem) {
  return showingSchema.value && row.note_id === noteId.value && row.schema_index === schemaIndex.value
}

function isNoteSelected(row: NoteDiagramListItem) {
  return showingNote.value && row.note_id === noteId.value
}

function applyDetailToList(current: NoteDiagramListItem, next: NoteDiagramDetail) {
...
async function loadList() {
  ...
  error.value = ''
  try {
    items.value = await diagramsApi.list()
  } catch (e) {
...
async function loadDetail() {
  ...
  saveError.value = ''
  try {
    detail.value = await diagramsApi.get(noteId.value, schemaIndex.value)
...
async function onSceneChange(scene: string) {
  ...
  saveError.value = ''
  try {
    detail.value = await diagramsApi.update(current.note_id, current.schema_index, {
      scene,
...
function openSchema(row: NoteDiagramListItem) {
...
function openNote(id: string) {
  void router.push({ name: 'diagram-note', params: { noteId: id } })
}
...
function onNoteClose() {
  ...
  void router.push({ name: 'diagrams' })
}
  ...
  <div class="workspace">
    ...
    <div class="split">
      <button
        ...
        title="Развернуть список диаграмм"
        aria-label="Развернуть список диаграмм"
      ...
      <aside v-else class="list-pane" :style="listPaneStyle">
        <div class="list-toolbar">
          <div class="search-row">
            <input
              ...
              aria-label="Поиск по имени диаграммы"
            />
            <input
              v-model="qNote"
              type="search"
            ...
            <button
              type="button"
              class="btn-list-hide"
              title="Скрыть список диаграмм"
              aria-label="Скрыть список диаграмм"
              @click="toggleListCollapsed"
            >
              −
            </button>
        ...
        <div class="cols-head" :style="colsStyle">
          <div class="col-title">Диаграмма</div>
          <div
            class="col-split"
            title="Потяните, чтобы изменить ширину названия диаграммы"
        ...
        <p v-else-if="!filtered.length" class="empty">
          Нет диаграмм по этому фильтру. Диаграмма появляется здесь, когда она есть в доступной заметке.
        </p>
        <ul v-else class="rows" role="listbox" aria-label="Диаграммы">
          <li
      ...
      <section class="preview-pane">
        <template v-if="showingNote">
          <NoteEditorColumn
            :note-id="noteId"
            embedded
            embedded-back-label="← К диаграммам"
            @close="onNoteClose"
        ...
        <template v-else-if="showingSchema">
          <div class="preview-bar">
            ...
            <VisualBlockSceneActions
              ...
              :can-export="!!auth.user?.can_export_diagrams"
              accept=".drawio,.xml,text/xml,application/xml"
              reload-title="Перезагрузить диаграмму, если загрузка зависла"
          ...
          <p v-if="detailLoading" class="muted pad">Загрузка диаграммы…</p>
          <p v-else-if="!detail" class="err pad">{{ error || 'Диаграмма не найдена' }}</p>
          <DrawioStandaloneEditor
            v-else
            ref="sceneEditorRef"
        ...
        <div v-else class="preview-empty">
          <p>Выберите диаграмму слева — она откроется здесь.</p>
