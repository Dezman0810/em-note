...
import { errMessage, schemasApi } from '../api/client'
import type { NoteSchemaDetail, NoteSchemaListItem } from '../api/types'
...
const LIST_COLLAPSED_KEY = 'note-ui-schemas-list-collapsed'
const LIST_WIDTH_KEY = 'note-ui-schemas-list-width'
const SCHEMA_COL_KEY = 'note-ui-schemas-col-schema'
const LIST_MIN = 220
const LIST_MAX = 760
...
const items = ref<NoteSchemaListItem[]>([])
...
function isSchemaSelected(row: NoteSchemaListItem) {
  return showingSchema.value && row.note_id === noteId.value && row.schema_index === schemaIndex.value
}

function isNoteSelected(row: NoteSchemaListItem) {
...
async function loadList() {
  ...
  error.value = ''
  try {
    items.value = await schemasApi.list()
  } catch (e) {
...
async function loadDetail() {
  ...
  try {
    ...
    lastSchema.value = { noteId: noteId.value, index: schemaIndex.value }
  } catch (e) {
    error.value = errMessage(e)
    detail.value = null
  } finally {
...
function applyDetailToList(current: NoteSchemaListItem, next: NoteSchemaDetail) {
...
function openSchema(row: NoteSchemaListItem) {
  lastSchema.value = { noteId: row.note_id, index: row.schema_index }
  void router.push({
    name: 'schema-edit',
...
function openNote(id: string) {
  void router.push({ name: 'schema-note', params: { noteId: id } })
}

function onNoteClose() {
  const back = lastSchema.value
  ...
  <div class="workspace">
    ...
    <div class="split">
      <button
        ...
        title="Развернуть список схем"
        aria-label="Развернуть список схем"
      ...
      <aside v-else class="list-pane" :style="listPaneStyle">
        <div class="list-toolbar">
          <div class="search-row">
            <input
              ...
              class="search"
              placeholder="Схема"
              aria-label="Поиск по имени схемы"
            />
            <input
              v-model="qNote"
              type="search"
            ...
            <button
              type="button"
              class="btn-list-hide"
              title="Скрыть список схем"
              aria-label="Скрыть список схем"
              @click="toggleListCollapsed"
            >
              −
            </button>
        ...
        <div class="cols-head" :style="colsStyle">
          <div class="col-title">Схема</div>
          <div
            class="col-split"
            title="Потяните, чтобы изменить ширину названия схемы"
        ...
        <p v-else-if="!filtered.length" class="empty">
          Нет схем по этому фильтру. Схема появляется здесь, когда она есть в доступной заметке.
        </p>
        <ul v-else class="rows" role="listbox" aria-label="Схемы">
          <li
      ...
      <section class="preview-pane">
        ...
        <template v-else-if="showingSchema">
          ...
          <p v-if="detailLoading" class="muted pad">Загрузка схемы…</p>
        ...
        <div v-else class="preview-empty">
          <p>Выберите схему слева — она откроется здесь.</p>
