...
async def _load_note(db: AsyncSession, note_id: uuid.UUID) -> Note | None:
    # Метки нужны для tag_ids в ответе; note.shares никто не читает, а каскадное
    # удаление само догружает коллекцию внутри flush.
...
async def get_note_access(
    ...
    """Заметка с загруженными метками + права запрашивающего. Одна загрузка на запрос."""
    note = await _load_note(db, note_id)
    if note is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Note not found")

    # Владелец по-прежнему «видит» заметку в корзине; редактирование отключает ensure_note_editable.
...
def ensure_note_editable(note: Note, access: Access) -> Note:
    """Проверки редактирования по уже загруженной заметке — без повторного SELECT."""
...
def ensure_note_owner(note: Note, access: Access) -> Note:
...
async def require_note_edit(
    ...
    return ensure_note_editable(note, access)
...
async def require_note_owner(
    ...
    note, access = await get_note_access(db, note_id, user_id)
    return ensure_note_owner(note, access)
