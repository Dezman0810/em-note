...
from app.schemas.note_mindmap import NoteMindmapDetail, NoteMindmapListItem, NoteMindmapPatch
from app.services.note_access import Access, ensure_note_editable, get_note_access
from app.services.note_mindmaps import (
    mindmap_caption,
    mindmap_node_count,
    notes_with_mindmap_candidates,
    replace_mindmap_attrs,
    walk_mindmap_nodes,
)
...
def _item_from_node(
    *,
    ...
    scene: str | None = None,
) -> NoteMindmapListItem | NoteMindmapDetail:
    raw_scene = scene if scene is not None else scene_from_node(node)
    payload = {
        "note_id": note_id,
        "note_title": note_title,
        "schema_index": schema_index,
        "block_id": block_id_from_node(node),
        "caption": mindmap_caption(raw_scene, schema_index, node),
        "element_count": mindmap_node_count(raw_scene),
    ...
    if scene is not None:
        return NoteMindmapDetail(**payload, scene=raw_scene)
    return NoteMindmapListItem(**payload)


@router.get("", response_model=list[NoteMindmapListItem])
async def list_mindmaps(
    db: Annotated[AsyncSession, Depends(get_db)],
    user: Annotated[User, Depends(require_schemas_access)],
) -> list[NoteMindmapListItem]:
    """Карты только из заметок, которые пользователь уже может открыть."""
    result = await db.execute(
        notes_with_mindmap_candidates(user.id).options(selectinload(Note.tags))
    )
    ...
    items: list[NoteMindmapListItem] = []
    for note in notes:
        ...
        for index, node in enumerate(walk_mindmap_nodes(doc)):
            items.append(
                _item_from_node(
                    note_id=note.id,
...
@router.get("/{note_id}/{schema_index}", response_model=NoteMindmapDetail)
async def get_mindmap(
    ...
    doc = parse_tiptap(note.content_json)
    nodes = walk_mindmap_nodes(doc) if doc else []
    ...
    return _item_from_node(
        ...
        scene=scene_from_node(node),
    )


@router.patch("/{note_id}/{schema_index}", response_model=NoteMindmapDetail)
async def patch_mindmap(
    note_id: uuid.UUID,
    schema_index: int,
    body: NoteMindmapPatch,
    db: Annotated[AsyncSession, Depends(get_db)],
    user: Annotated[User, Depends(require_schemas_access)],
) -> NoteMindmapDetail:
    if schema_index < 0:
