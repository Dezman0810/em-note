...
from app.schemas.note_schema import NoteSchemaDetail, NoteSchemaListItem, NoteSchemaPatch
...
from app.services.note_schemas import (
    ...
    block_id_from_node,
    dump_tiptap,
    notes_with_schema_candidates,
    parse_tiptap,
    replace_schema_attrs,
    scene_from_node,
    schema_caption,
    schema_element_count,
    walk_excalidraw_nodes,
)
...
def _item_from_node(
    *,
    ...
    scene: str | None = None,
) -> NoteSchemaListItem | NoteSchemaDetail:
    ...
    payload = {
        ...
        "caption": schema_caption(raw_scene, schema_index, node),
        "element_count": schema_element_count(raw_scene),
    ...
    if scene is not None:
        return NoteSchemaDetail(**payload, scene=raw_scene)
    return NoteSchemaListItem(**payload)


@router.get("", response_model=list[NoteSchemaListItem])
async def list_schemas(
    db: Annotated[AsyncSession, Depends(get_db)],
    user: Annotated[User, Depends(require_schemas_access)],
) -> list[NoteSchemaListItem]:
    """Схемы только из заметок, которые пользователь уже может открыть."""
    result = await db.execute(
        notes_with_schema_candidates(user.id).options(selectinload(Note.tags))
    )
    ...
    for note in notes:
        ...
        for index, node in enumerate(walk_excalidraw_nodes(doc)):
            items.append(
                _item_from_node(
                    note_id=note.id,
...
@router.get("/{note_id}/{schema_index}", response_model=NoteSchemaDetail)
async def get_schema(
    ...
    doc = parse_tiptap(note.content_json)
    nodes = walk_excalidraw_nodes(doc) if doc else []
...
@router.patch("/{note_id}/{schema_index}", response_model=NoteSchemaDetail)
async def patch_schema(
    note_id: uuid.UUID,
    schema_index: int,
    body: NoteSchemaPatch,
    db: Annotated[AsyncSession, Depends(get_db)],
    user: Annotated[User, Depends(require_schemas_access)],
) -> NoteSchemaDetail:
    if schema_index < 0:
